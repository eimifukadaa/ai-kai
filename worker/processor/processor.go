...
func (p *Processor) extractTextWithOCR(pdfPath string, pageNum int) (string, error) {
	// Create a unique temp dir for this page to avoid conflicts
	pageTempDir, err := os.MkdirTemp("", fmt.Sprintf("ocr_page_%d_", pageNum))
	if err != nil {
		return "", fmt.Errorf("failed to create temp dir: %w", err)
	}
	defer os.RemoveAll(pageTempDir) // Cleanup everything on return

	// Extract page as image using pdfcpu (pure Go)
	// This works well for scanned PDFs where pages are images
	conf := model.NewDefaultConfiguration()
	
	// pdfcpu extract requires string page numbers
	// It extracts images embedded in the page to the output directory
	err = api.ExtractImagesFile(pdfPath, pageTempDir, []string{fmt.Sprintf("%d", pageNum)}, conf)
	if err != nil {
		return "", fmt.Errorf("failed to extract images from page %d: %w", pageNum, err)
	}
	
	// Find the extracted image in the temp dir
	files, err := os.ReadDir(pageTempDir)
	if err != nil {
		return "", fmt.Errorf("failed to read temp dir: %w", err)
	}
	
	if len(files) == 0 {
		// No images found on this page? Maybe it's not a scanned PDF but just has weird text encodings?
		return "", nil // No images to OCR
	}
	
	// Use the first extracted image file
	imgPath := filepath.Join(pageTempDir, files[0].Name())
	...
	outputPath := filepath.Join(pageTempDir, "ocr_output") // tesseract adds .txt extension
