...
export default function ChatPage() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    if (res.ok) {
      const conversationList: Conversation[] = await res.json();
  ...
  const loadConversation = useCallback(async () => {
    ...
    if (res.ok) {
      const data: { title: string; messages?: StoredMessage[] } =
        await res.json();
  ...
  const latestData =
    data && data.length > 0 ? (data[data.length - 1] as ChatDataPayload) : null;
  const citations = latestData?.citations || [];

  useEffect(() => {
    const model = latestData?.model;
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2">
          <p className="text-xs text-gray-500 uppercase font-bold mb-2 px-2">
            Chat History
          </p>
      ...
      <main className="flex-1 flex flex-col w-full relative bg-white">
        <header className="h-16 bg-white border-b flex items-center px-4 justify-between shrink-0">
          <button
            ...
            className="p-2 md:hidden text-gray-900 hover:bg-gray-100 rounded-md"
          >
                ...
                citations.map((cite, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 text-indigo-600 font-medium mb-1">
                      ...
                      <span>
                        {cite.document_name || "Dokumen"} - Page{" "}
                        {cite.page_number}
                      </span>
                    </div>
